...
interface SetupOpts {
  ...
  /** Next-engine export payload. Defaults to a single `public.players` table. */
  exportFiles?: ReadonlyArray<{ name: string; sql: string }>;
}
...
function setup(workdir: string, opts: SetupOpts = {}) {
  ...
  const nextExportFiles = opts.exportFiles ?? [
    { name: "public/tables/players.sql", sql: "create table players ();" },
  ];
  const exportSchemaCalls: Array<ReadonlyArray<string>> = [];
  const engine =
      ...
      ? Layer.succeed(
          ...
          LegacyPgDeltaEngine.of({
            ...
            exportDeclarativeSchema: (input) =>
              Effect.sync(() => {
                exportSchemaCalls.push(input.schema);
                return {
                  files: nextExportFiles,
...
describe("legacy db schema declarative generate integration", () => {
  ...
  it.effect(
    "writes the public schema a developer would edit: enum, table, auth view, and extensions",
    () => {
      const s = setup(tmp.current, {
        experimental: true,
        engineImplementation: "next",
        exportFiles: [
          {
            name: "_cluster/extensions/uuid-ossp.sql",
            sql: 'CREATE EXTENSION "uuid-ossp" SCHEMA "extensions";\n',
          },
          {
            name: "public/types/account_state.sql",
            sql: "CREATE TYPE public.account_state AS ENUM ('pending', 'active');\n",
          },
          {
            name: "public/tables/dogfood_note.sql",
            sql: "CREATE TABLE public.dogfood_note (\n  id bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,\n  body text NOT NULL,\n  CONSTRAINT dogfood_note_pkey PRIMARY KEY (id)\n);\n\nCREATE INDEX dogfood_note_loop_status_idx ON public.dogfood_note USING btree (loop_status);\n",
          },
          {
            name: "public/views/auth_user_emails.sql",
            sql: "CREATE VIEW public.auth_user_emails AS SELECT id, email FROM auth.users;\n",
          },
        ],
      });
      return Effect.gen(function* () {
        ...
        expect(
          readFileSync(join(schemas, "public", "tables", "dogfood_note.sql"), "utf8"),
        ).toContain("GENERATED BY DEFAULT AS IDENTITY");
        expect(
          readFileSync(join(schemas, "public", "tables", "dogfood_note.sql"), "utf8"),
        ).toContain("CREATE INDEX dogfood_note_loop_status_idx");
        expect(
          readFileSync(join(schemas, "public", "types", "account_state.sql"), "utf8"),
        ).toContain("ENUM");
        expect(
          readFileSync(join(schemas, "public", "views", "auth_user_emails.sql"), "utf8"),
        ).toContain("auth.users");
        expect(
          readFileSync(join(schemas, "_cluster", "extensions", "uuid-ossp.sql"), "utf8"),
        ).toContain("uuid-ossp");
        expect(existsSync(join(schemas, "auth", "tables", "users.sql"))).toBe(false);
        expect(existsSync(join(schemas, "storage"))).toBe(false);
        expect(existsSync(join(schemas, "realtime"))).toBe(false);
        expect(s.resolverCalls).toContainEqual(expect.objectContaining({ connType: "linked" }));
      }).pipe(Effect.provide(s.layer));
    },
  );

  it.effect("keeps _custom SQL when regenerating an existing declarative tree", () => {
    const schemas = join(tmp.current, "supabase", "schemas");
    mkdirSync(join(schemas, "_custom"), { recursive: true });
    writeFileSync(join(schemas, "_custom", "casts.sql"), "create cast (int as text);\n");
    const s = setup(tmp.current, {
      experimental: true,
      engineImplementation: "next",
      yes: true,
    ...
    return Effect.gen(function* () {
      yield* legacyDbSchemaDeclarativeGenerate(
        flags({ local: Option.some(true), overwrite: true }),
      );
      expect(readFileSync(join(schemas, "_custom", "casts.sql"), "utf8")).toBe(
        "create cast (int as text);\n",
      );
      expect(existsSync(join(schemas, "public", "tables", "players.sql"))).toBe(true);
    }).pipe(Effect.provide(s.layer));
  });

  it.effect(
    "forwards --schema to the next engine so generate can restrict to those schemas",
    () => {
      const s = setup(tmp.current, { experimental: true, engineImplementation: "next" });
      return Effect.gen(function* () {
        yield* legacyDbSchemaDeclarativeGenerate(
          flags({ local: Option.some(true), schema: ["public"] }),
        );
        expect(s.exportSchemaCalls).toEqual([["public"]]);
      }).pipe(Effect.provide(s.layer));
    },
  );
});
